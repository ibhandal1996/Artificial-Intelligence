...
public class Layer {
	...
	public void computeOutputDelta() {
		...
		for(int i =0; i < n; i++)
			delta.set(i, (targ.difference(act).get(i) * net.derivative(min, max).get(i)));
		//deltaSet(i) = (Target(i) - activationLevel(i)) * the derivative of activationFunction
		//(Target(i) - activationLevel(i)) could be called an error check
	}
	...
	public void computeHiddenDelta() {
		...
		Vector trans = new Vector(n, 0); //creates a new vector to fill
		
		for(Projection oProj : outputs) //go through all of the projection that could be an output
		{
			for(int i = 0; i < n; i++)
			{
				delta.set(i, trans.sum(oProj.W.transpose().product(oProj.output.delta)).get(i) * net.derivative(min, max).get(i));
				//deltaSet(i) = (SumOf[delta(j) * weight(i)(j)]) * the derivative of activationFunction
				//The (SumOf[delta(j) * weight(i)(j)]) is basically a vector multiplied by a matrix
			}
		}
	}
