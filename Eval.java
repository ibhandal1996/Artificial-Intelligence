...
public class Eval {
	...
	static double value_roll_hand(State s, int depth) {
		...
		for(DieFace DIE1 : DieFace.values()) //Goes through all outcomes of the 1st die
		{
			if(DIE1 != DieFace.invalid) //checks if the value of DIE1 is valid or not
			{
				for(DieFace DIE2 : DieFace.values())//Goes through all outcomes of the 2nd die
				{
					if(DIE2 != DieFace.invalid) //checks if the value for DIE2 id valid or not
					{
						for(DieFace DIE3 : DieFace.values())//Goes through all outcomes of the 3rd die
						{
							if(DIE2 != DieFace.invalid) //checks if the value for DIE3 id valid or not
							{
								//if all is valid then there is a roll
								s.roll(DIE1, DIE2, DIE3);
								vrh = value_rolled_hand(s, depth);
								rp = s.rollProb(DIE1, DIE2, DIE3);
								val = (rp * vrh) + val; //adds the roll probability with the value of the hand
							}
						}
					}
				}
	...
	static public double heuristic(State s) {
		...
		if(s.blasts_collected < 2) //if-statement that checks if the blast collected are less than 2
		{
			if(s.comp_brains_eaten > s.user_brains_eaten) //if statement that checks if the computer has eaten more brains than the player
			{
			value = State.win_payoff; //this makes the value positive if these statements are true
			}
		}
		else
		{
			value = -(State.win_payoff); //this makes the value negative if the if statements are false
		}
		return (value); //return the value that was produced
	}
