...
public class BackwardChain {
	...
	public BindingList unify(Literal lit1, Literal lit2, BindingList bl) {
		...
		if(lit1.pred.equals(lit2.pred) == true) //compares the predicates and id they equal one another t
		{                                       //then the unify function is used 
			return unify(lit1.args,lit2.args,bl); //args == t1 or t2
		}
	...
	public BindingList unify(Term t1, Term t2, BindingList bl) {
		...
		if(t1.c != null && t2.c != null) //t1 and t2 are both constants
		{
			if((t1.equals(t2)) == true) //compares the constants and if they are the same
			{                           //a new bl is returned
		...
		else if(t1.c != null && t2.v != null) //t1 is a constant and t2 is a variable
		{
			return unify(t2,t1,bl); //Unifies the variable with the constant
		}
		
		else if(t1.v != null && t2.c != null) //t1 is a variable and t2 is a constant
		{
			bl.addBinding(t1.v, t2); //this bounds the variable and constant
			return new BindingList(bl);
		}
		
		else if(t1.v != null && t2.v != null) //t1 and t2 are both variables
		{
			bl.addBinding(t1.v, t2); //this bounds the two variables together
			return new BindingList(bl);
		}
		
		else if(t1.v != null && t2.f != null) //t1 is a variable and t2 is a function
		{
			if((t2.f.allVariables().contains(t1.v))== true)
			{
				return (null); //if there is any matches then the unify function cannot be used
			}
			bl.addBinding(t1.v, t2); //this binds the variable to the function
			return new BindingList(bl);
		}
		
		else if(t1.f != null && t2.v != null) //t1 is a function and t2 is a variable
		{
			return unify(t2,t1,bl); //Unifies the variable with the function
		}
		
		else if(t1.f != null && t2.f != null) //t1 and t2 are both functions
		{
			return unify(t1.f,t2.f,bl); //Unifies the functions
		}
		
		return (null); //if any of these cases are not met (t1 being a constant 
					   //and t2 being a function is also dealt here and the opposite as well
					   //because you can't unify a function and a constant together)
	}
	...
	public BindingList unify(Function f1, Function f2, BindingList bl) {
		...
		if((f1.func.equals(f2.func))== true) //the functions are compared and if they equal one another
		{                                    //then the unify function can be used
			return unify(f1.args,f2.args,bl); //args == t1 or t2
		}
